...
class ImportStory extends Command
{
     ...
     * @var string
     */
    protected $signature = 'portchris:story:import {story}';
     ...
     * @var string
     */
    protected $description = 'Import story by file';
    ...
    {
        $return = $file = "";
        $id = $this->argument('story');
        $n = $this->option('no-interaction');
        $verbosity = OutputInterface::VERBOSITY_NORMAL;
        try {
            if (!$id || strlen($id) <= 0) {
                $str = "Error: Please specify a story to import!";
                $return .= $str;
                throw new InvalidArgumentException($str);
            }
            $slug = ($id == Page::PAGE_HOMEPAGE) ? "/" : $id;
            $id .= self::FILE_EXT;
            $this->info("BEGIN: Portchris Story Import: {$id}!", $verbosity);
            $this->setPageSlug($slug);
            $this->setPageId($slug);
            if (File::exists($this->importPath . $id)) {
                $str = $this->output("Found file of ID: %s.", $id);
                $return .= $str;
                $this->info($str, $verbosity);

                // Truncate content_metas table
                if (strpos($id, self::START_UP) !== false) {
                    $str = " " . $this->output("Truncating messages table.");
                    $return .= $str;
                    $this->info($str, $verbosity);
                ...
                $str = $this->output("Cannot find file: %s%s", $this->importPath, $id);
                $return .= $str;
                throw new InvalidArgumentException($str);
            }
            $this->info("END: Portchris Story Import: {$id}!", $verbosity);
        } catch (InvalidArgumentException $e) {
            $this->error($this->output("%s", $e->getMessage()), $verbosity);
        } catch (Exception $e) {
            $this->error($this->output("%s", $e->getMessage()), $verbosity);
        }
        return $return;
